...
with open('result.csv', 'wb') as csvfile:
    fieldnames = ['key', 'Date', 'Time', 'IssuerName', 'SecurityName', 'GroupCode',  'CategoryName', 'AnnTitle']
    my_writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    my_writer.writeheader()
    for item in items:
        filtered_item = {k: item.get(k) for k in tuple(fieldnames)}
        my_writer.writerow(filtered_item)
